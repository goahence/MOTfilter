...
from geometry.nusc_distance import iou_bev, iou_3d, giou_bev, giou_3d, d_eucl, giou_3d_s, fast_giou_3d
...
class Tracker:
    ...
    def compute_cost(self) -> dict:
        ...
        tra_cost_infos = {'np_dets': self.tra_infos['np_tras'][:, :-3],
                          'np_dets_bottom_corners': self.tra_infos['np_tras_bottom_corners'],
                          'np_dets_norm_corners': self.tra_infos['np_tras_norm_corners']}

        if self.fast:
            # metrics only have giou_3d/giou_bev
            first_func = 'giou_3d' if 'giou_3d' in self.re_metrics else 'fast_giou_3d'
            two_cost, first_cost = globals()[first_func](self.det_infos, tra_cost_infos)
