...
import pdb
import cv2
from detection_class import project_points_to_image
import numpy as np
...
def yolo_tracker(polyloader,polytracker,track_result):
    # PolyMOT modal is completely dependent on the detector modal
    # tracking and output file
    # pdb.set_trace()
    result = track_result
    nusc_loader = polyloader
    nusc_tracker = polytracker
    # nusc_loader2[0]

    frame_data = nusc_loader.get_result()
    if frame_data:
        sample_token = frame_data['sample_token']
        # track each sequence
        # pdb.set_trace()
        ...
        # output process
        sample_results = []
        #should be writen as a dict list
        target_box = []
        target_id = []
        target_rotate = []
        target_center = []
        ...
        if 'no_val_track_result' not in frame_data:
            for predict_box in frame_data['box_track_res']:
                ...
                sample_results.append(box_result.copy())
                target_box.append(predict_box.corners().T)
                target_id.append(predict_box.tracking_id)
                target_rotate.append(box_result["rotation"])
                target_center.append(box_result["translation"])
                final_info.append({'box_corner': predict_box.corners().T, 
                                   'id': predict_box.tracking_id, 
                                   'wxyz':box_result["rotation"], 
                                   'center':box_result["translation"], 
                                   'size': box_result['size']})
        # pdb.set_trace()   
        # add to the output file
        if sample_token in result["results"]:
            result["results"][sample_token] = result["results"][sample_token] + sample_results
    ...
    # pdb.set_trace()
    # print(f'tracking_id:{result["results"][sample_token]}')
    return final_info
