...
from .motion_model import CA, CTRA, BICYCLE,CTRA2
...
class KalmanFilter:
    ...
    def getMeasureInfo(self, det: dict = None) -> np.array:
        ...
        if GEO_FILTER: del list_det[2:6]
        ...
        if not GEO_FILTER:
            ...
            assert len(list_det) == 5 if self.has_velo else 3
    ...
    def addFrameObject(self, timestamp: int, tra_info: dict, mode: str = None) -> None:
        ...
        box_info, bm_info, norm_bm_info = arraydet2box(exter_info, np.array([self.tracking_id]))
...
class ExtendKalmanFilter(KalmanFilter):
    def __init__(self, timestamp: int, config: dict, track_id: int, det_infos: dict) -> None:
        ...
        self.model = globals()[self.model](self.has_velo, self.dt) if self.model in ['CTRA', 'BICYCLE','CTRA2'] \
